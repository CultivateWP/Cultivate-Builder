...
namespace Cultivate\Blocks\Social_Links;
...
function site( $block = [] ) {
	...
	foreach ( $socials as $social => $settings ) {
		...
		if ( ! empty( $url ) ) {
			$output[] = '<li><a href="' . esc_url_raw( $url ) . '" target="_blank" rel="noopener noreferrer" aria-label="' . $settings['label'] . '">' . cwp_icon( array( 'icon' => $icon ) ) . '</a></li>';
		}
	}
